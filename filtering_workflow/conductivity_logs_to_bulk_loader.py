...
outdir = r"\\prod.lan\active\proj\futurex\Common\Working\Neil\filtered_borehoel_gfx\induction\data"
...
data_dir = r'\\prod.lan\active\proj\futurex\Common\Working\Neil\filtered_borehoel_gfx\induction\data'

for file in glob.glob(os.path.join(data_dir, '*.csv')):
    path, fname = os.path.split(file)
    well = fname.split('_DEEP')[0].split('_INDUCTION')[0]
    ...
    df_well.rename(columns = {'filtered_S/m': 'VALUE'}, inplace = True)
    # add instrument
    instrument = row['induction_instrument'].values[0]
    df_well['INSTRUMENT'] = instrument
    df_bulk_loader = df_bulk_loader.append(df_well)


df_bulk_loader['PROPERTY'] = 1
df_bulk_loader['UOM'] = 'S/m'
...
df_bulk_loader.reset_index().to_csv('EFTF1_conductivity_bulk_loader.csv', index = False)